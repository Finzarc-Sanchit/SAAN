...
function SplitWord({ word, className, ariaHidden }: SplitWordProps) {
  return (
    <h2
      className={cn(
    ...
    >
      {word.split('').map((letter, index) => (
        <span key={`${letter}-${index}`} className="inline-block min-w-[0.55em] text-center">
          {letter}
        </span>
      ))}
    </h2>
...
export function AtelierClassicHeroSection() {
  const { word, tagline, ariaLabel, portrait } = ATELIER_COPY.classic;
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 select-none" aria-hidden>
        <div className="absolute top-[10%] left-1/2 -translate-x-1/2">
          <p className="text-[clamp(4rem,18vw,13rem)] leading-none font-bold tracking-tight text-[#ececec] uppercase">
            {word}
          </p>
        </div>
      ...
      <div className="relative z-10 flex min-h-[340px] items-center justify-center sm:min-h-[420px] lg:min-h-[480px]">
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="flex w-full justify-center" aria-hidden>
            <SplitWord word={word} className="text-[#1a1a1a]" ariaHidden />
          </div>
        </div>
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="relative h-[min(52vw,420px)] w-[min(38vw,300px)] overflow-hidden sm:h-[420px] sm:w-[300px]">
            <div className="absolute top-1/2 left-1/2 flex w-screen -translate-x-1/2 -translate-y-1/2 justify-center">
              <SplitWord word={word} className="text-white" ariaHidden />
            </div>
          </div>
        </div>
